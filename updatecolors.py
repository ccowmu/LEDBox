    ...
    try:
        while True:
            ...
            if status['type'] != "random":
                color = Color(int(status['red']), int(status['green']), int(status['blue']))

            rand = random.randint(1, 3)

            if status['type'] == "rainbow" or (status['type'] == "random" and rand == 1):
                rainbowCycle(strip)
            elif status['type'] == "chase" or (status['type'] == "random" and rand == 2):
